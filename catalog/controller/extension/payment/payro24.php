...
class ControllerExtensionPaymentpayro24 extends Controller
{
    ...
    */
    ...
    {
        return 'payro24 Transaction ID: ' . $id;
    }
    ...
    */
    public function index()
    {
        ...
        $sandbox = $this->config->get('payro24_sandbox') == 'yes' ? 'true' : 'false';
        ...
        if (extension_loaded('curl')) {
            ...
            $api = $this->config->get('payro24_api_key');
            //$callback = $this->url->link('extension/payment/payro24/callback', 'order_id=' . $encryption->encrypt($order_info['order_id']), '', 'SSL');
            $callback = $this->url->link('extension/payment/payro24/callback', 'order_id=' . $order_info['order_id']);
            ...
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, 'https://api.payro24.ir/v1.0/payment');
            ...
            if ($http_status != 201 || empty($result) || empty($result->id) || empty($result->link)) {
                ...
                $model->addOrderHistory($order_id, 1, 'در حال هدایت به درگاه پرداخت پیرو', false);
        ...
        return $this->load->view('/extension/payment/payro24.tpl', $data);
    }

    /**
    * callback payro24 http request
    */
    public function callback()
    {
        ...
        if (!$order_info) {
            $comment = $this->payro24_get_failed_message($track_id, $order_id);
            ...
            if ($status != 10) {
                $comment = $this->payro24_get_failed_message($track_id, $order_id, $status);
                $data['error_warning'] = $comment;
                ...
                $amount = $this->correctAmount($order_info);
                $api_key = $this->config->get('payro24_api_key');
                $payro24_data = array(
                    'id' => $id,
                    'order_id' => $order_id,
                );
                $ch = curl_init();
                curl_setopt($ch, CURLOPT_URL, 'https://api.payro24.ir/v1.0/payment/verify');
                curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($payro24_data));
                curl_setopt($ch, CURLOPT_RETURNTRANSFER, TRUE);
                curl_setopt($ch, CURLOPT_HTTPHEADER, array(
                    'Content-Type: application/json',
                    'P-TOKEN:' . $api_key,
                ...
                if ($http_status != 200) {
                    ...
                    if (empty($verify_status) || empty($verify_track_id) || empty($verify_amount) || $verify_amount != $amount || $verify_status < 100) {
                        $comment = $this->payro24_get_failed_message($verify_track_id, $verify_order_id);
                        // Set Order status id to 10 (Failed) and add a history.
                        $model->addOrderHistory($order_id, 10, $comment, true);
                        $data['error_warning'] = $comment;
                        $data['button_continue'] = $this->language->get('button_view_cart');
                        ...
                        $comment = $this->payro24_get_failed_message($track_id, $order_id, 0);
                        $model->addOrderHistory($order_id, 10, $this->otherStatusMessages($status), true);
                        $data['error_warning'] = $comment;
                        $data['button_continue'] = $this->language->get('button_view_cart');
                        $data['continue'] = $this->url->link('checkout/cart');
                        ...
                        $comment = $this->payro24_get_success_message($verify_track_id, $verify_order_id);
                        $config_successful_payment_status = $this->config->get('payro24_order_status_id');
        ...
        $this->response->setOutput($this->load->view('/extension/payment/payro24_callback.tpl', $data));
    }
    ...
    */
    private function payro24_get_success_message($track_id, $order_id)
    {
        return str_replace(["{track_id}", "{order_id}"], [$track_id, $order_id], $this->config->get('payro24_payment_successful_message'));
    }
    ...
    * @return string
    */
    private function payro24_get_failed_message($track_id, $order_id, $msgNumber = null)
    {
        $msg = $this->otherStatusMessages($msgNumber);
        $msg = str_replace(["{track_id}", "{order_id}"], [$track_id, $order_id], $this->config->get('payro24_payment_failed_message')) . $msg;
        return $msg;
    }
